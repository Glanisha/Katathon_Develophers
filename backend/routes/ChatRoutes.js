...
// post message to chat and optionally generate assistant reply
router.post('/:id/message', auth, async (req, res) => {
  try {
    const { text, generateReply = true, replyContext = null } = req.body;
    if (!text) return res.status(400).json({ error: 'Text required' });

    const result = await ChatService.handleUserMessage(req.params.id, req.user, text, { generateReply, replyContext });
    res.json(result);
  } catch (err) {

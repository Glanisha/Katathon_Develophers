...
router.post('/request', auth, async (req, res) => {
  try {
    ...
    if (!query || !query.trim()) return res.status(400).json({ message: 'Invalid query' });
    ...
    if (toUser._id.equals(me._id)) return res.status(400).json({ message: "You cannot add yourself" });
    if (me.friends.includes(toUser._id)) return res.status(400).json({ message: 'Already friends' });
    ...
    if (existing) return res.status(400).json({ message: 'There is already a pending friend request' });

    const reqDoc = new FriendRequest({ from: me._id, to: toUser._id });
    await reqDoc.save();

    console.log('Friend request created:', { from: me._id, to: toUser._id, requestId: reqDoc._id });
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(friendId)) {
      console.error('Remove friend: invalid id', { friendId, userId: req.user && req.user.id, authHeader: req.headers?.authorization });
      return res.status(400).json({ message: 'Invalid friend id' });
    }
    const me = await User.findById(req.user.id);
    if (!me) {
      console.error('Remove friend: user not found', { userId: req.user && req.user.id, authHeader: req.headers?.authorization });
      return res.status(401).json({ message: 'User not found' });
    }
    const friend = await User.findById(friendId);

    if (!friend) return res.status(404).json({ message: 'Friend not found' });
    console.log('Remove friend request', { userId: me._id.toString(), friendId, authHeader: req.headers?.authorization });

    // remove references both ways
    me.friends = me.friends.filter((f) => f.toString() !== friendId);
    friend.friends = friend.friends.filter((f) => f.toString() !== me._id.toString());
    ...
    await FriendRequest.updateMany({
      $or: [ { from: me._id, to: friend._id }, { from: friend._id, to: me._id } ]
    }, { status: 'declined' });
...
// Get outgoing friend requests (sent by me) — MUST come before /requests/:id routes
router.get('/requests/sent', auth, async (req, res) => {
  try {
    console.log('Fetching sent requests for user:', req.user.id);
    const outgoing = await FriendRequest.find({ from: req.user.id, status: 'pending' }).populate('to', 'name email');
    console.log('Found sent requests:', outgoing.length, outgoing);
...
router.delete('/requests/:id', auth, async (req, res) => {
  try {
    ...
    if (requestDoc.status !== 'pending') return res.status(400).json({ message: 'Request already handled' });
...
router.post('/requests/:id/approve', auth, async (req, res) => {
  try {
    ...
    console.log('Friend request approved:', { from: requestDoc.from, to: req.user.id, bothNowFriends: true });
